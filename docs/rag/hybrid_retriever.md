...
Given:
- the `VectorStoreOutput` from a dense FAISS search, and
- the `Bm25RetrieverOutput` from a BM25 keyword search
...
4. Returns the top-k, ready for FlashRank reranking (SCRUM-259).
...
for hc in result.top_chunks:
    print(f"Rank {hc.rank}  rrf={hc.rrf_score:.5f}  "
...
| Type | Fields |
|---|---|
| `HybridRetrieverInput` | `dense_results: VectorStoreOutput`, `bm25_results: Bm25RetrieverOutput`, `top_k: int = 5` |
| `HybridRetrieverOutput` | `top_chunks: list[HybridRetrievedChunk]`, `total_unique` |
| `HybridRetrievedChunk` | `chunk: ChunkMetadata`, `rrf_score`, `dense_rank: int \| None`, `bm25_rank: int \| None`, `rank` |
...
only, semantic match only, or both).
